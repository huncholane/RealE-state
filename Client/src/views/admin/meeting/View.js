import { Box, Button, Flex, Grid, GridItem, Heading, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Text } from '@chakra-ui/react';
...
const View = () => {
    ...
    return (
        <>
                            ...
                            <Card>
                                <Grid gap={4}>
                                    ...
                                    <GridItem colSpan={{ base: 2, md: 1 }}>
                                        ...
                                        <Text>
                                            {/* this was a typo, createdByName -> createByName to keep the same as others */}
                                    ...
                                    <GridItem colSpan={{ base: 2, md: 1 }}>
                                        ...
                                        {data?.related === 'Contact' && contactAccess?.view
                                            ...
                                            ? data?.attendes &&
                                              data?.attendes.map((item) => {
                                                  return <Text color="blackAlpha.900">{item.firstName + ' ' + item.lastName}</Text>;
                                              })
                                            : data?.related === 'lead'
                                            ? data?.attendesLead &&
                                              data?.attendesLead.map((item) => {
                                                  return <Text color="blackAlpha.900">{item.leadName}</Text>;
                                              })
                                            : '-'}
                                    </GridItem>
                            ...
                            <Grid templateColumns="repeat(6, 1fr)" gap={1}>
                                <GridItem colStart={6}>
                                    <Flex justifyContent={'right'}>
                                        ...
                                        {user.role === 'superAdmin' || permission?.delete ? (
                                            <Button size="sm" style={{ background: 'red.800' }} onClick={() => setDeleteMany(true)} leftIcon={<DeleteIcon />} colorScheme="red">
                                                Delete
                                            </Button>
                                        ) : (
                                            ''
            ...
            <AddMeeting isOpen={edit} fetchData={fetchData} onClose={() => setEdit(false)} userAction={'edit'} id={params.id} init={data} setData={setData} />
        </>
    );
};
