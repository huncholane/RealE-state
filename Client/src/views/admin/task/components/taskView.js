                    ...
                    <Card>
                        <Grid gap={4}>
                            <GridItem colSpan={2}>
                                <Box>
                                    <Box display={'flex'} justifyContent={'space-between'}>
                                        ...
                                        <Flex id="hide-btn">
                                            <Menu>
                                                {(user.role === 'superAdmin' || permission?.create || permission?.update || permission?.delete) && (
                                                ...
                                                <MenuList minWidth={2}>
                                                    {(user.role === 'superAdmin' || permission?.create) && (
                                                        <MenuItem onClick={() => handleClick()} alignItems={'start'} color={'blue'} icon={<AddIcon />}>
                                                            Add
                                                        </MenuItem>
                                                    )}
                            ...
                            <GridItem colSpan={{ base: 2, md: 1 }}>
                                ...
                                {editableField === 'title' ? (
                                    <>
                                        <Input id="text" name="title" type="text" onChange={formik.handleChange} onBlur={handleBlur} value={formik.values.title} borderColor={formik?.errors?.title && formik?.touched?.title ? 'red.300' : null} autoFocus />
                            ...
                            <GridItem colSpan={{ base: 2, md: 1 }}>
                                ...
                                {editableField === 'start' ? (
                                    <>
                                        <Input name="start" type={data?.allDay ? 'date' : 'datetime-local'} onChange={formik.handleChange} onBlur={handleBlur} value={data.allDay === true ? moment(formik.values.start).format('YYYY-MM-DD') : moment(formik.values.start).format('YYYY-MM-DD HH:mm A')} autoFocus borderColor={formik?.errors?.start && formik?.touched?.start ? 'red.300' : null} min={data?.allDay ? dayjs(today).format('YYYY-MM-DD') : dayjs(todayTime).format('YYYY-MM-DD HH:mm')} />
                                        <Text mb="10px" color={'red'}>
                                            {' '}
                                            {formik?.errors.start && formik?.touched.start && formik?.errors.start}
                                        </Text>
                                    </>
                                ) : (
                                    <Text onDoubleClick={() => handleDoubleClick('start', data?.start)}>{data && data?.start ? (data.allDay === true ? moment(data.start).format('DD-MM-YYYY') : moment(data.start).format('DD-MM-YYYY HH:mm A')) : '-'}</Text>
                                )}
                            </GridItem>
                            <GridItem colSpan={{ base: 2, md: 1 }}>
                                <Text fontSize="sm" fontWeight="bold" color={'blackAlpha.900'}>
                            ...
                            <GridItem colSpan={{ base: 2, md: 1 }}>
                                ...
                                <Link to={data?.assignTo ? contactAccess?.view && `/contactView/${data?.assignTo}` : leadAccess?.view && `/leadView/${data?.assignToLead}`}>
                                    <Text
                                        color={data?.category === 'contact' && (contactAccess?.view || user?.role === 'superAdmin') ? 'brand.600' : leadAccess?.view || (user?.role === 'superAdmin' && data?.category === 'lead') ? 'brand.600' : 'blackAlpha.900'}
                                        sx={{
                                            '&:hover': {
                                                color: 'blue.500',
                            ...
                            <GridItem colSpan={{ base: 2 }}>
                                ...
                                {editableField === 'description' ? (
                                    <>
                                        <Input id="text" name="description" type="text" onChange={formik.handleChange} onBlur={handleBlur} value={formik.values.description} autoFocus borderColor={formik?.errors?.description && formik?.touched?.description ? 'red.300' : null} />
                                        <Text mb="10px" color={'red'}>
                                            {' '}
                            ...
                            <GridItem colSpan={{ base: 2 }}>
                                ...
                                {editableField === 'notes' ? (
                                    <>
                                        <Input id="text" name="notes" type="text" onChange={formik.handleChange} onBlur={handleBlur} value={formik.values.notes} autoFocus borderColor={formik?.errors?.notes && formik?.touched?.notes ? 'red.300' : null} />
                                        <Text mb="10px" color={'red'}>
                                            {' '}
                        ...
                        <GridItem colStart={6}>
                            <Flex justifyContent={'right'}>
                                {(permission?.update || user?.role === 'superAdmin') && (
                                    <Button size="sm" onClick={() => setEdit(true)} leftIcon={<EditIcon />} mr={2.5} variant="outline" colorScheme="green">
                                        Edit
                                    </Button>
                                )}
                                {(permission?.delete || user?.role === 'superAdmin') && (
                                    <Button size="sm" style={{ background: 'red.800' }} onClick={() => setDeleteManyModel(true)} leftIcon={<DeleteIcon />} colorScheme="red">
                                        Delete
                                    </Button>
                                )}
                            </Flex>
